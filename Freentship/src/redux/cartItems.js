  ...
  reducers: {
    addToCart(state, action) {
      ...
      const find = state.findIndex((item) => item.storeId === storeId);
        if (find !== -1) {
          const findItem = state[find].items.findIndex((item) => item.idFood === idFood);
            if (findItem !== -1) {
              state[find].items[findItem].Quantity += Quantity;
            } else {
                state[find].items.push({idFood,title,description,image,price, Quantity})
            }
      } else state.push({ storeId, storeName, storeAddress, storeImageParams, items:  [{idFood,title,description,image,price, Quantity}]});
